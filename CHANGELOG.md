...
19-09-2016
----------
выяснить как в NodeJS создавать подомены вида beta.gdetus.io | m.gdetus.io
(решено с помощью плагина `vhost`)

Решить - будет ли новая архитектура подразумевает, что приложение будет SinglePageApplication  + API
(решено, будет: приложение будет SinglePageApplication  + API)



1-09-2016
----------
реализован функционал выставления, изменения и показа\скрытия даты рождения юзера в профиле
мелкие правки

календарь возраста юзера:
Исправить сохранение значения "галочка выключена" для поля birthday_date_muted
Сейчас он может сохранять лишь значение "галочка включена" (решено)



31-08-2016
----------
решён баг с невозможностью логина после истечения срока сессии

написана первая версия даты рождения пользователя, вывода её в шаблон и функционала запрещения
её вывода пользователем на страницу своего профиля


29-08-2016
----------
сделать вывод аватара юзера на страницу карты в метку пользователя (решено)

костыльно решен вопрос размещения кнопок смены аватара и фона юзера
...
24-08-2016
----------
решён старый баг с НЕвозвратом на свою страницу при клике на свое имя если при регистрации
не указан useruri или он удален

решен вопрос загрузки аватара и фона пользователя
*в данный момент загрузка того и другого происходит ОДИН РАЗ на одну операцию. то
есть - если не обновить страницу и попытаться загрузить новый аватар или фон
ВТОРИЧНО - загрузки не происходит + теоретически сервер может остановиться с ошибкой
(ситуация была однажды в песочнице)

решен баг с неотображением в хедере аватара юзера на всех страницах кроме профиля

сделать (помимо аватара) загрузку фона юзера. с перезаписью файла в БД в случае
    если страницу браузера не перезагружали после аплоада.
    
запретить загрузку аватара простым перетаскиванием файла на браузер с открытой
страницей профиля пользователя. копипаст файлов уже отключен. (решено)
   
назначить аватар по умолчанию каждому юзеру, который отображается до момента когда 
юзер сам выбирает и заливает свой собственный
   
необходимо чтобы генерация пароля при его восстановлении проходила по правилам
создания паролей (от 8 символов, включая спецсимволы, буквы A-z и цифры)
...
19-08-2016
----------
созданы требования к созданию паролей:
     1.1) при регистрации                                       (решено)
     1.2) при восстановлении (в методе User.generatePassword()) (решено)
     1.3) при смене пароля самим пользователем:                 (решено)
    
     - длина пароля должна составлять: не менее 8 символов (числа, англ. буквы)
     - включая минимум один спецсимвол (%*]`~) и т.д.)
     
гитигнор: туду, бета, ридми

раздел "безопасность": создана форма смены пароля пользователем, правила проверки полей
...
Сделаны отдельные облачные команды : старт\стоп программы на боевом сервере.
